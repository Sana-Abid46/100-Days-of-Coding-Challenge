...
function Cars(model:string, manufacturer:string, color?:string,  aditionalInfo?: string) {
    let carInfo : {model:string, manufacturer:string, color?:string, aditionalInfo?: string} = {model,manufacturer}
    if(color) {
        carInfo.color = color
    }
    if(aditionalInfo) {
